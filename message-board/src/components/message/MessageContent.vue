  ...
  <div class="message-content">
    <template v-if="messageData?.length === 0">
      <div class="no-data">
        ...
        <p class="no-data-text">还没有留言,快发布第一条吧！</p>
      </div>
    </template>
    <template v-else>
      <MessageChild
...
const messageData = ref<Array<IMessage> | []>([]);

watch(
  () => props.msg,
  (newValue, oldValue) => {
    if (newValue !== oldValue) retrieveData(newValue);
  }
);

// 重新获取数据
const retrieveData = async (key: string) => {
  const { data } = await axios.post(url + "/table/getMessageContent", {
    data: {
      key,
    },
  });
...
onMounted(() => {
  retrieveData(props.msg);
});
</script>

<style scoped lang="sass">
@import url("../../style/components/messageContent.scss")
</style>
